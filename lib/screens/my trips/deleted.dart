...
class _DeletedState extends State<Deleted> {
    ...
    if (currentUser != null) {
      ...
      try {
        ...
        await tripDocRef.update({
          'status': 1, // Restore trip by setting status to 1
        });
  ...
  Future<void> permanentlyDeleteTrip(BuildContext context, String tripName) async {
    ...
    if (currentUser != null) {
      ...
      try {
        ...
        await tripDocRef.delete(); // Permanently delete the trip document
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  List<Member> getMembers(Map<String, dynamic> tripData) {
    // Implement this method to retrieve members from tripData
    List<Member> members = [];
    return members;
  }
}
